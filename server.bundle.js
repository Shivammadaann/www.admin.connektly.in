...
var clientApiBaseUrl = process.env.CLIENT_API_BASE_URL || "";
var clientApiHealthUrl = process.env.CLIENT_API_HEALTH_URL || "";
var websitePublicBaseUrl = normalizeWebsitePublicBaseUrl(process.env.WEBSITE_PUBLIC_BASE_URL || process.env.WEBSITE_API_BASE_URL);
var websiteApiBaseUrl = normalizeWebsiteApiBaseUrl(process.env.WEBSITE_API_BASE_URL || `${websitePublicBaseUrl}/api`);
var websiteContentSyncToken = process.env.WEBSITE_CONTENT_SYNC_TOKEN || "";
var websiteContentRoot = process.env.WEBSITE_CONTENT_ROOT ? path.resolve(process.env.WEBSITE_CONTENT_ROOT) : "";
var allowedEmails = new Set(
  (process.env.ADMIN_ALLOWED_EMAILS || "").split(",").map((value) => value.trim().toLowerCase()).filter(Boolean)
);
var allowedUserIds = new Set(
  (process.env.ADMIN_ALLOWED_USER_IDS || "").split(",").map((value) => value.trim()).filter(Boolean)
);
var ownerProfileBucket = process.env.OWNER_PROFILE_BUCKET || "owner-admin-profile-pictures";
var primaryOwnerEmail = (process.env.PRIMARY_OWNER_EMAIL || "admin@connektly.in").trim().toLowerCase();
var adminInviteRedirectUrl = process.env.ADMIN_INVITE_REDIRECT_URL || "";
var graphVersion = process.env.META_GRAPH_VERSION || "v24.0";
var metaWebhookVerifyToken = process.env.META_WEBHOOK_VERIFY_TOKEN || "";
var messengerWebhookVerifyToken = process.env.MESSENGER_WEBHOOK_VERIFY_TOKEN || "";
var tokenEncryptionSecret = process.env.META_TOKEN_ENCRYPTION_KEY || "";
var tokenEncryptionKey = tokenEncryptionSecret ? crypto.createHash("sha256").update(tokenEncryptionSecret).digest() : null;
...
var smtpFrom = process.env.SMTP_FROM || smtpUser || "Connektly Admin <no-reply@connektly.in>";
var adminAccessRequestEmail = process.env.ADMIN_ACCESS_REQUEST_EMAIL || "Admin@connektly.in";
var hasRequiredServerConfig = Boolean(supabaseUrl && supabaseAnonKey && supabaseServiceRoleKey);
var anonSupabase = createClient(supabaseUrl || "https://placeholder.supabase.co", supabaseAnonKey || "placeholder-key");
var adminSupabase = createClient(supabaseUrl || "https://placeholder.supabase.co", supabaseServiceRoleKey || "placeholder-key", {
  auth: {
    autoRefreshToken: false,
    persistSession: false
  }
});
var liveClients = /* @__PURE__ */ new Map();
var realtimeStatus = "not-started";
...
function normalizeWebsitePublicBaseUrl(value) {
...
function normalizeWebsiteApiBaseUrl(value) {
...
function isRecent(value, hours) {
  ...
  return Date.now() - timestamp <= hours * 60 * 60 * 1e3;
}
function formatTableName(table) {
  return table.split("_").map((segment) => segment.charAt(0).toUpperCase() + segment.slice(1)).join(" ");
}
...
async function requireAdmin(req, res, next) {
  try {
    ...
    if (!access) {
      res.status(403).json({
        error: `This account is not allowed to use the Admin Control Centre. Ask ${primaryOwnerEmail} to invite this user from Admin Profile > User Management.`
      });
      return;
    }
    req.admin = {
...
async function safeSelect(table, select = "*", build) {
  try {
...
async function safeRpc(functionName, args) {
  try {
...
function userDisplayName(profile, authUser) {
  return normalizeString(profile?.full_name) || normalizeString(authUser?.user_metadata?.full_name) || normalizeString(authUser?.user_metadata?.name) || normalizeString(profile?.company_name) || normalizeString(authUser?.email) || "Unknown user";
}
...
function detectDevice(userAgent, fallback) {
...
function normalizeAuthEventType(value) {
...
function extractAuditUserAgent(rawPayload, row) {
...
function extractAuditIpAddress(rawPayload, row) {
  const traits = nestedRecord(rawPayload, "traits");
  const metadata = nestedRecord(rawPayload, "metadata");
  const request = nestedRecord(rawPayload, "request");
  const requestHeaders = nestedRecord(request, "headers");
...
function extractAuditDevice(rawPayload, row, userAgent) {
  const traits = nestedRecord(rawPayload, "traits");
  const metadata = nestedRecord(rawPayload, "metadata");
  const request = nestedRecord(rawPayload, "request");
  return detectDevice(
...
async function loadUserLoginActivity(userId, authUser) {
...
var allAdminPermissions = adminPermissionCatalog.map((permission) => permission.key);
    ...
    customTriggers: [
      {
        ...
        html: '<div style="font-family: Arial, sans-serif; color: #111827; line-height: 1.6;"><h2>{{title}}</h2><p>{{body}}</p><p>Template: {{template_name}}</p><p><a href="{{target_url}}">Open templates in Connektly</a></p></div>',
...
function normalizeOwnerNotifications(value) {
...
function normalizeAdminPermissions(value, role) {
  if (role === "primary_owner") {
    return allAdminPermissions;
  }
  const allowed = new Set(allAdminPermissions);
  const incoming = Array.isArray(value) ? value : [];
  return incoming.map((item) => normalizeString(item)).filter((item) => Boolean(item && allowed.has(item)));
}
...
function buildAdminAccessContext(row, source) {
...
async function upsertPrimaryOwnerAccess(user) {
  ...
  const { data, error } = await adminSupabase.from("owner_admin_users").upsert(payload, { onConflict: "email" }).select("*").single();
  if (error) {
...
async function getAdminAccessForUser(user) {
  const email = normalizeEmail(user.email);
  ...
  if (byUserId.error && isMissingRelationError(byUserId.error)) {
    const legacyAllowed = allowedUserIds.has(user.id) || (email ? allowedEmails.has(email) : false) || process.env.NODE_ENV !== "production" && allowedEmails.size === 0 && allowedUserIds.size === 0;
...
async function getOwnerProfileRow(adminUserId) {
  const { data, error } = await adminSupabase.from("owner_admin_profiles").select("*").eq("admin_user_id", adminUserId).maybeSingle();
  if (error) {
    if (isMissingRelationError(error)) {
      return {
        row: null,
...
async function upsertOwnerProfile(admin, updates) {
  const payload = {
  ...
  const { data, error } = await adminSupabase.from("owner_admin_profiles").upsert(payload, { onConflict: "admin_user_id" }).select("*").single();
  if (error) {
    if (isMissingRelationError(error)) {
      return {
        row: payload,
...
var platformSettingSections = Object.keys(defaultPlatformSettings);
function cloneDefaultPlatformSettings() {
  return JSON.parse(JSON.stringify(defaultPlatformSettings));
}
...
function normalizePlatformSettingsSection(section, incoming, current) {
  ...
  if (section === "pricing_plans") {
    ...
    return {
      plans: plans.map((plan) => ({
        ...
        features: Array.isArray(plan.features) ? plan.features.map((feature) => normalizeString(feature)).filter(Boolean) : [],
  ...
  if (section === "api_keys") {
    const currentById = new Map((Array.isArray(current?.keys) ? current.keys : []).map((key) => [String(key.id), key]));
  ...
  if (section === "email_templates") {
    ...
    return {
      ...
      customTriggers: customTriggers.map((trigger) => {
        const triggerKey = normalizeString(trigger.triggerKey) || normalizeString(trigger.actionName)?.toLowerCase().replace(/[^a-z0-9]+/g, "_") || `custom_action_${Date.now()}`;
...
function redactPlatformSettingsForAdmin(settings) {
  return {
    ...settings,
    api_keys: {
      keys: (settings.api_keys.keys || []).map((apiKey) => ({
        ...apiKey,
        key: "",
...
async function savePlatformSettingsSection(admin, section, incoming) {
...
function buildPublicPricingPlans(settings) {
...
async function updateAdminUserMetadata(admin, updates) {
...
function buildOwnerSettings(admin, profile, warning, authUser = admin.user) {
  const metadata = isRecord(authUser.user_metadata) ? authUser.user_metadata : {};
  const fullName = normalizeString(profile?.full_name) || normalizeString(metadata.full_name) || normalizeString(metadata.name) || normalizeString(admin.email?.split("@")[0]) || "Owner";
  const organizationName = normalizeString(profile?.organization_name) || normalizeString(metadata.organization_name) || normalizeString(metadata.company_name) || "Connektly";
  return {
    profile: {
      ...
      avatarUrl: normalizeString(profile?.avatar_url) || normalizeString(metadata.avatar_url) || normalizeString(metadata.picture) || null,
      organizationName,
...
function buildAdminUserRow(row, authUser) {
  const role = normalizeAdminRole(row.role);
  const status = normalizeAdminStatus(row.status);
  return {
    ...
    fullName: normalizeString(row.full_name) || normalizeString(authUser?.user_metadata?.full_name) || normalizeString(authUser?.user_metadata?.name) || normalizeEmail(row.email)?.split("@")[0] || "Admin user",
...
async function loadDashboardAdmins() {
  ...
  const authByEmail = new Map(authUsers.map((user) => [normalizeEmail(user.email), user]).filter(([email]) => Boolean(email)));
  return {
...
function normalizeInvitePermissions(value) {
  ...
  return permissions.length > 0 ? permissions : ["command_center"];
}
function parseProfilePhotoData(dataUrl) {
...
function buildUserRows(args) {
  ...
  return [...allIds].map((userId) => {
    ...
    const credits = args.creditLedger.filter((row) => row.user_id === userId).reduce((total, row) => {
...
function rowOwnerUserId(row) {
  return normalizeString(row.user_id) || normalizeString(row.workspace_owner_user_id) || normalizeString(row.owner_user_id);
}
function normalizePaymentAmount(row) {
  ...
  for (const key of amountKeys) {
    ...
    if (!numeric) continue;
    const looksLikeMinorUnit = key.includes("amount") && numeric >= 1e3;
    return looksLikeMinorUnit ? numeric / 100 : numeric;
  }
  return 0;
}
function normalizeRazorpayTimestamp(value) {
...
function normalizePaymentMethodLabel(method, cardType) {
...
async function fetchRazorpayJson(pathname, query) {
  ...
  if (!response.ok) {
    const errorMessage = isRecord(payload) && isRecord(payload.error) ? normalizeString(payload.error.description) || normalizeString(payload.error.reason) : null;
    throw new Error(errorMessage || `Razorpay API request failed with status ${response.status}.`);
  }
  return payload;
}
function latestPaidInvoice(invoices) {
  const items = isRecord(invoices) && Array.isArray(invoices.items) ? invoices.items.filter(isRecord) : [];
  return items.filter((item) => normalizeString(item.payment_id)).sort((left, right) => normalizeNumber(right.created_at) - normalizeNumber(left.created_at))[0] || null;
}
function mapRazorpayPaymentMethod(payment, invoice, subscription = null, error) {
  ...
  const upi = isRecord(payment?.upi) ? payment.upi : {};
  const token = isRecord(payment?.token) ? payment.token : {};
  const cardLast4 = normalizeString(card.last4) || normalizeString(payment?.card_last4) || normalizeString(payment?.card_last_four);
  ...
  const amountPaid = normalizeRazorpayAmount(payment?.amount) || normalizeRazorpayAmount(invoice?.amount_paid) || normalizePaymentAmount(payment || {}) || normalizePaymentAmount(invoice || {});
  const paymentDate = normalizeRazorpayTimestamp(payment?.created_at) || normalizeRazorpayTimestamp(invoice?.paid_at) || normalizeRazorpayTimestamp(invoice?.created_at);
  const nextDueDate = normalizeRazorpayTimestamp(subscription?.charge_at) || normalizeRazorpayTimestamp(subscription?.current_end) || normalizeRazorpayTimestamp(subscription?.end_at) || normalizeRazorpayTimestamp(invoice?.next_payment_at) || null;
  const tokenId = normalizeString(payment?.token_id) || normalizeString(token.id);
  const mandateEnabled = Boolean(
    tokenId || payment?.recurring === true || subscription?.status === "active" || normalizeString(subscription?.auth_status) === "authorized"
  );
  return {
    method: method || null,
...
async function resolveSubscriptionPaymentMethod(subscriptionId) {
...
async function enrichProfilesWithPaymentMethods(profiles) {
...
function monthStart(offsetFromCurrent) {
...
function dayStart(offsetFromCurrent) {
  const date = /* @__PURE__ */ new Date();
  date.setUTCHours(0, 0, 0, 0);
...
function logStatus(row) {
  return normalizeString(row.status) || normalizeString(row.delivery_status) || normalizeString(row.processing_status) || normalizeString(row.payment_status) || normalizeString(row.state) || "recorded";
}
function logErrorType(row) {
  const value = [
    row.error_type,
    row.error_code,
    row.error,
...
function logDetail(row) {
  return normalizeString(row.error_message) || normalizeString(row.failure_reason) || normalizeString(row.error) || normalizeString(row.message) || normalizeString(row.title) || null;
}
function logOccurredAt(row) {
  return normalizeString(row.created_at) || normalizeString(row.updated_at) || nowIso();
}
function pickLogString(row, keys) {
...
function metaContext(featureName, permissionName, endpoint, method) {
  return {
    isMetaApi: true,
...
function nonMetaContext(endpoint, method) {
  return {
    isMetaApi: false,
    apiProvider: null,
    apiEndpoint: endpoint || null,
...
function metaApiContextForText(text, endpoint, method) {
...
function metaApiContextForPath(pathname, method) {
...
function metaApiContextForTable(table, row = {}, endpointOverride, methodOverride) {
...
function metaApiContextForLiveEvent(event) {
...
function mapWhatsAppTwoStepVerification(row) {
  const metadata = getMetadataRecord(row);
  ...
  const liveIsEnabled = typeof twoStep.isPinEnabled === "boolean" ? twoStep.isPinEnabled : normalizeString(twoStep.codeVerificationStatus)?.toUpperCase() === "VERIFIED" ? true : null;
  const enabledAtMs = Date.parse(String(enabledAt || ""));
  const disabledAtMs = Date.parse(String(disabledAt || ""));
  const inferredEnabled = Boolean(enabledAt) && (!Number.isFinite(disabledAtMs) || !Number.isFinite(enabledAtMs) || enabledAtMs >= disabledAtMs);
  return {
...
function mapWhatsAppVerificationCodeRequest(row) {
...
function mapWhatsAppSenderRegistration(row) {
...
function mapWhatsAppDisplayName(row) {
  const metadata = getMetadataRecord(row);
...
function mapWhatsAppChannel(row) {
  if (!row) return null;
  return {
    id: normalizeString(row.id) || "",
...
function buildOrganizationRows(core, authUsers) {
  ...
  return users.map((user) => {
    ...
    const ownerMatches = (row) => rowOwnerUserId(row) === ownerUserId;
    ...
    const unpaidHighUsage = ["free", "trialing", "none", ""].includes(String(plan).toLowerCase()) && (messages.length > 500 || threads.length > 100 || callLogs.length > 100 || emailCampaigns.length > 50);
...
function buildOrganizationDetail(core, authUsers, orgId) {
  ...
  return {
    ...
    usageStats: {
      ...
      apiUsage: core.messages.filter(ownerMatches).length + core.callSessions.filter(ownerMatches).length + core.leadEvents.filter(ownerMatches).length + core.paymentEvents.filter(ownerMatches).length
    },
...
function metaErrorMessage(payload) {
  const payloadRecord = isRecord(payload) ? payload : {};
  if (isRecord(payloadRecord.error)) {
    const error = payloadRecord.error;
    return normalizeString(error.error_user_msg) || normalizeString(error.message) || normalizeString(isRecord(error.error_data) ? error.error_data.details : null);
  }
  return null;
}
async function metaRequest(args) {
...
async function getOrgWhatsAppChannel(orgId) {
...
function getAdminWebhookCallbackUrl(req) {
...
async function persistWhatsAppWebhookSubscriptionStatus(args) {
  ...
  }).eq("user_id", args.orgId).eq("id", args.row.id).select("*").single();
  if (error) throw error;
  return data;
}
async function listWhatsAppSubscribedApps(accessToken, wabaId) {
  return metaRequest({ accessToken, path: `${wabaId}/subscribed_apps` });
}
async function subscribeWhatsAppWebhook(accessToken, wabaId, req) {
...
async function unsubscribeWhatsAppWebhook(accessToken, wabaId) {
  return metaRequest({ accessToken, path: `${wabaId}/subscribed_apps`, method: "DELETE" });
}
async function runWhatsAppWebhookAction(orgId, action, req) {
...
async function requestWhatsAppVerificationCode(orgId, codeMethod, language) {
  const { row, accessToken } = await getOrgWhatsAppChannel(orgId);
  ...
  const { data, error } = await adminSupabase.from("meta_channels").update({
    ...
    updated_at: timestamp
  }).eq("user_id", orgId).eq("id", row.id).select("*").single();
  if (error) throw error;
  return data;
}
function summarizeHealth(dbLatencyMs, clientHealth) {
  const envChecks = [
...
async function checkClientApiHealth() {
  ...
  const healthUrls = /* @__PURE__ */ new Set();
...
function getEventTitle(table, row, eventType) {
...
function getEventSeverity(table, row) {
...
function mapLivePayload(table, payload) {
  const row = isRecord(payload.new) ? payload.new : isRecord(payload.old) ? payload.old : {};
  ...
  return {
    ...
    description: normalizeString(row.last_message_text) || normalizeString(row.body) || normalizeString(row.error_message) || normalizeString(row.description) || normalizeString(row.campaign_name) || void 0,
    severity: getEventSeverity(table, row),
    status: normalizeString(row.processing_status) || normalizeString(row.status) || normalizeString(row.state) || normalizeString(row.type),
    payload: row
  };
}
...
async function recordAdminAudit(admin, action, targetUserId, metadata) {
  const event = {
    ...
    title: action.split("_").map((segment) => segment.charAt(0).toUpperCase() + segment.slice(1).toLowerCase()).join(" "),
    description: admin?.email ? `By ${admin.email}` : void 0,
    severity: "success",
    status: "completed",
    payload: metadata
  };
...
async function loadCoreData() {
  const [
    ...
    safeSelect("app_profiles", "*", (query) => query.order("created_at", { ascending: false }).limit(1e3)),
    ...
    safeSelect("meta_flows", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
    safeSelect("flow_submissions", "*", (query) => query.order("submitted_at", { ascending: false }).limit(1e3)),
    safeSelect("meta_conversational_automation_configs", "*"),
    safeSelect("automation_rules", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
    safeSelect("conversation_threads", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
    safeSelect("conversation_messages", "*", (query) => query.order("created_at", { ascending: false }).limit(1e3)),
    safeSelect("call_logs", "*", (query) => query.order("created_at", { ascending: false }).limit(1e3)),
    safeSelect("call_sessions", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
    safeSelect("meta_lead_capture_events", "*", (query) => query.order("created_at", { ascending: false }).limit(500)),
    safeSelect(
      "whatsapp_payment_configuration_events",
      "*",
      (query) => query.order("created_at", { ascending: false }).limit(500)
    ),
    safeSelect("email_campaigns", "*", (query) => query.order("created_at", { ascending: false }).limit(1e3)),
    safeSelect("credit_ledger", "*", (query) => query.order("created_at", { ascending: false }).limit(2e3)),
    safeSelect("user_notifications", "*", (query) => query.order("created_at", { ascending: false }).limit(500)),
    safeSelect("user_notification_preferences", "*"),
    safeSelect("email_connections", "*"),
    safeSelect("email_templates", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
    safeSelect("woocommerce_connections", "*"),
    safeSelect("developer_api_credentials", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
    safeSelect("developer_webhook_endpoints", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
    safeSelect("workspace_team_members", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
    safeSelect("meta_templates", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3))
  ]);
  return {
...
function buildTimeline(core) {
  const items = [
    ...
    ...core.paymentEvents.slice(0, 30).map((row) => mapLivePayload("whatsapp_payment_configuration_events", { eventType: "INSERT", new: row })),
  ...
  return items.sort((left, right) => Date.parse(right.occurredAt) - Date.parse(left.occurredAt)).slice(0, 60);
}
function buildClientApiUrl(pathname) {
  if (!clientApiBaseUrl) {
    return null;
  }
...
function normalizeWebhookList(value, fallback = []) {
  if (Array.isArray(value)) {
...
function isActiveWebhookRow(row) {
...
function tokenPreview(source, scope, values) {
  const tokens = values.map((value) => normalizeString(value)).filter((value) => Boolean(value));
...
function noWebhookToken(source = "Not required") {
  return {
    source,
    scope: "external",
    hasToken: false,
...
function activeLeadConfigTokenValues(leadConfigs) {
  return leadConfigs.filter((row) => isActiveWebhookRow(row)).map((row) => normalizeString(row.verify_token)).filter((value) => Boolean(value));
}
function webhookEndpointUrl(row) {
  return normalizeString(row.url) || normalizeString(row.endpoint_url) || normalizeString(row.callback_url) || normalizeString(row.target_url) || normalizeString(row.webhook_url) || normalizeString(row.delivery_url);
}
function webhookEndpointSecret(row) {
  return normalizeString(row.signing_secret) || normalizeString(row.webhook_secret) || normalizeString(row.secret) || normalizeString(row.token) || normalizeString(row.verify_token);
}
function webhookEndpointMethods(row) {
...
function webhookEndpointEvents(row) {
  return normalizeWebhookList(row.events || row.event_types || row.subscribed_events || row.topics, ["workspace events"]);
}
function developerWebhookReference(row, index) {
  const url = webhookEndpointUrl(row);
  if (!url || !isActiveWebhookRow(row)) {
    return null;
  }
  const actualRowId = normalizeString(row.id);
  const rowId = actualRowId || crypto.createHash("sha1").update(`${url}:${normalizeString(row.name) || normalizeString(row.label) || index}`).digest("hex").slice(0, 16);
  ...
  return {
    ...
    purpose: normalizeString(row.description) || normalizeString(row.purpose) || "Customer-configured developer webhook endpoint for workspace events.",
...
function inferConversationWebhookIds(row) {
  const haystack = [
    row.channel,
    row.channel_type,
    row.provider,
...
function inferWebhookIds(table, row = {}) {
...
function liveEventWebhookIds(event) {
...
function enrichWebhookEvent(event) {
...
function isFailedWebhookEvent(event) {
...
function withWebhookStats(webhook, events) {
  ...
  const lastEventAt = relatedEvents.map((event) => event.occurredAt).sort((left, right) => Date.parse(right) - Date.parse(left))[0] || null;
  return {
    ...webhook,
...
async function loadWebhookTokenValues(webhookId) {
  ...
  if (webhookId === "meta-lead-capture") {
    const configs = await safeSelect(
      "meta_lead_capture_configs",
      "*",
      (query) => query.order("updated_at", { ascending: false }).limit(500)
    );
    return configs.rows.filter((row) => isActiveWebhookRow(row)).map(
      (row, index) => webhookTokenValue({
        id: normalizeString(row.id) || `meta-lead-token:${index}`,
        label: normalizeString(row.page_name) || normalizeString(row.form_name) || normalizeString(row.user_id) || `Lead capture workspace ${index + 1}`,
  ...
  if (webhookId.startsWith("developer:")) {
    ...
    const endpoint = await safeSelect("developer_webhook_endpoints", "*", (query) => query.eq("id", endpointId).limit(1));
...
function buildWebhookReferences(args = {}) {
  ...
  const references = [
    {
      ...
      purpose: "GET verifies Meta subscription challenges. POST receives inbound WhatsApp messages, message delivery/read statuses, WhatsApp call events, and payment configuration update events.",
    ...
    },
    {
      ...
      purpose: "GET verifies Page webhook subscription challenges. POST receives Messenger page messages, postbacks, reads, deliveries, and echo events.",
    ...
    },
    {
      ...
      methods: ["GET", "POST"],
      provider: "Meta Messenger / Facebook Pages",
      purpose: "Compatibility alias for the Messenger webhook. It uses the same verification and event handler as the canonical Messenger URL.",
      verifyTokenEnv: "MESSENGER_WEBHOOK_VERIFY_TOKEN",
      events: [
        "messages",
        "messaging_postbacks",
    ...
    },
    {
      ...
      purpose: "GET verifies leadgen webhook setup using each workspace verify token. POST receives page/form leadgen events and syncs them into CRM leads.",
    ...
    },
    {
      ...
      purpose: "This is not a fixed Connektly route. Each payment configuration stores its own provider data endpoint URL and sends it to Meta through the WhatsApp Payments setup flow.",
      events: [
      ...
      status: "external",
      notes: "Manage stored payment configuration URLs from WhatsApp Payments setup; webhook update events still arrive through the WhatsApp Cloud API webhook above.",
      eventCount: 0,
      successCount: 0,
      failureCount: 0,
      lastEventAt: null
  ...
  const developerReferences = (args.developerWebhookEndpoints || []).map((row, index) => developerWebhookReference(row, index)).filter((webhook) => Boolean(webhook));
  return [...references, ...developerReferences].map((webhook) => withWebhookStats(webhook, events));
}
function userActivityTimestamp(row, fallbackKey = "created_at") {
  return normalizeString(row.onboarding_completed_at) || normalizeString(row.completed_at) || normalizeString(row.paid_at) || normalizeString(row.payment_at) || normalizeString(row.expires_at) || normalizeString(row.trial_ends_at) || normalizeString(row[fallbackKey]) || normalizeString(row.updated_at) || nowIso();
}
function userActivityDetails(user, fallbackUserId, extra = {}) {
  return {
...
function isSuccessfulPaymentRow(row) {
  const value = [
    row.status,
    row.payment_status,
    row.event_type,
...
function buildUserActivityNotifications(core, authUsers) {
  const users = buildUserRows({
    authUsers,
    profiles: core.profiles,
    metaChannels: core.metaChannels,
  ...
  const now = Date.now();
  const signups = users.filter((user) => Boolean(user.createdAt)).map((user) => ({
  ...
  const onboarding = core.profiles.filter((row) => row.onboarding_completed === true || String(normalizeString(row.onboarding_status) || "").toLowerCase() === "completed").map((row) => {
    const userId = rowOwnerUserId(row);
    const user = userId ? userById.get(userId) : void 0;
  ...
  const trialExpiries = users.filter((user) => {
  ...
  const payments = core.paymentEvents.filter(isSuccessfulPaymentRow).map((row) => {
    const userId = rowOwnerUserId(row);
    const user = userId ? userById.get(userId) : void 0;
    ...
    return {
      ...
      metadata: {
        ...
        reference: normalizeString(row.razorpay_payment_id) || normalizeString(row.provider_payment_id) || normalizeString(row.payment_id) || normalizeString(row.id)
      }
    };
  });
  return [...signups, ...onboarding, ...trialExpiries, ...payments].sort((left, right) => Date.parse(String(right.occurredAt || 0)) - Date.parse(String(left.occurredAt || 0))).slice(0, 50);
}
function buildOverview(core, authUsers, health) {
  const organizations = buildOrganizationRows(core, authUsers);
  ...
  const monthlyRecurringRevenue = core.profiles.filter((row) => !isChurnedProfile(row)).reduce((total, row) => total + planMonthlyValue(row), 0);
  ...
  const planBreakdown = core.profiles.reduce((acc, row) => {
  ...
  return {
    ...
    metrics: {
      ...
      activeOrganizations: organizations.filter(
        (organization) => ["active", "paid", "trialing", "trial"].some((status) => String(organization.status || "").toLowerCase().includes(status))
      ).length,
    ...
    recentUsers: userRows.sort((left, right) => Date.parse(String(right.createdAt || 0)) - Date.parse(String(left.createdAt || 0))).slice(0, 8),
    warnings: core.errors
  };
}
function sortLogs(logs) {
  return logs.sort((left, right) => Date.parse(String(right.occurredAt)) - Date.parse(String(left.occurredAt)));
}
function buildLogsMonitoring(core, health) {
  const apiLogs = sortLogs([
    ...recentLiveEvents.filter((event) => event.eventType === "META_API_REQUEST" || event.source === "Meta Graph API").map((event) => {
    ...
    ...core.metaLeadCaptureConfigs.map(
      (row) => buildLogEntry({
        row,
  ...
  const webhookLogs = sortLogs([
    ...
    ...recentLiveEvents.filter((event) => event.table?.includes("webhook") || event.table === "conversation_messages" || event.table === "call_sessions").map((event) => {
      const meta = metaApiContextForLiveEvent(event);
      return {
        id: event.id,
        occurredAt: event.occurredAt,
  ...
  const errorLogs = sortLogs([
    ...apiLogs.filter((log) => log.errorType || log.severity === "critical").map((log) => ({ ...log, category: "error" })),
    ...webhookLogs.filter((log) => log.errorType || log.severity === "critical").map((log) => ({ ...log, category: "error" })),
    ...messageDeliveryLogs.filter((log) => log.errorType || log.severity === "critical").map((log) => ({ ...log, category: "error" })),
...
function integrationSeverity(total, unhealthy) {
  if (total === 0) return "warning";
  if (unhealthy === 0) return "success";
  return unhealthy >= total ? "critical" : "warning";
}
function latestTimestamp(rows) {
  const timestamps = rows.map((row) => timestampFromRow(row, "updated_at") || timestampFromRow(row, "created_at")).filter((date) => Boolean(date)).map((date) => date.getTime());
  return timestamps.length ? new Date(Math.max(...timestamps)).toISOString() : null;
}
function buildGlobalIntegrations(core, clientHealth) {
  ...
  return {
    ...
    integrations: [
      ...
      },
      {
        ...
        status: emailUnhealthy === 0 && failedEmailCampaigns === 0 && (core.emailConnections.length > 0 || core.emailCampaigns.length > 0) ? "healthy" : core.emailConnections.length === 0 && core.emailCampaigns.length === 0 ? "not configured" : "degraded",
...
var clientFeatureKeys = Object.keys(clientFeatureCatalog);
var clientFeatureStatusControls = {
...
function normalizeClientFeatureKey(value) {
  const key = normalizeString(value);
  return key && clientFeatureKeys.includes(key) ? key : null;
}
function groupRowsByOwner(rows, ownerResolver = rowOwnerUserId) {
...
function collectGroupOwnerIds(...groups) {
...
function getWorkspaceIdentity(userId, profileById, authById) {
  const profile = profileById.get(userId);
  const authUser = authById.get(userId);
  return {
    organizationName: normalizeString(profile?.company_name) || normalizeString(profile?.full_name) || normalizeString(authUser?.email) || compactUserId(userId),
...
function getFeatureSeverity(status, rows = []) {
  const text = [
    status,
    ...rows.flatMap((row) => [
      row.status,
...
function isHealthyFeatureStatus(status) {
  return getFeatureSeverity(status) === "success";
}
function collectFeatureRisks(rows, status, extras = []) {
  ...
  for (const row of rows) {
    const lastError = normalizeString(row.last_error) || normalizeString(row.error_message) || normalizeString(row.webhook_last_error) || normalizeString(row.failure_reason);
    if (lastError) {
      risks.add(lastError);
    }
  }
...
function countRowsWithStatus(rows, pattern) {
...
function latestByUpdatedAt(rows) {
  return rows.slice().sort(
    (left, right) => Date.parse(String(right.updated_at || right.created_at || right.submitted_at || 0)) - Date.parse(String(left.updated_at || left.created_at || left.submitted_at || 0))
  )[0] || null;
}
function buildClientFeatureOperations(core, authUsers) {
  const authById = new Map(authUsers.map((user) => [user.id, user]));
  ...
  for (const row of core.metaChannels) {
    ...
    addRecord({
      ...
      userId,
      status: normalizeString(row.status) || "connected",
      detail: normalizeString(row.display_phone_number) || normalizeString(row.verified_name) || normalizeString(row.phone_number_id) || "WhatsApp channel connected",
      rows: [row],
      metrics: [
  ...
  for (const row of core.metaAdsIntegrations) {
    ...
    addRecord({
      ...
      detail: normalizeString(row.ad_account_name) || normalizeString(row.ad_account_id) || normalizeString(row.page_name) || "Meta Ads integration saved",
      rows: [row],
      metrics: [
  ...
  for (const [userId, rows] of paymentEventsByUser) {
    ...
    addRecord({
      ...
      detail: normalizeString(latest?.configuration_name) || normalizeString(latest?.provider_name) || "WhatsApp payment configuration events recorded",
      rows,
      metrics: [
  ...
  for (const userId of collectGroupOwnerIds(emailConnectionsByUser, emailTemplatesByUser, emailCampaignsByUser)) {
    ...
    addRecord({
      ...
      detail: normalizeString(latestConnection?.email_address) || `${templates.length} templates and ${campaigns.length} campaigns without a saved mailbox`,
  ...
  const featureSummaries = clientFeatureKeys.map((featureKey) => {
    ...
    const severity = criticalCount > 0 ? "critical" : warningCount > 0 ? "warning" : configuredWorkspaceCount > 0 ? "success" : "warning";
    return {
      key: featureKey,
      label: catalog.label,
      category: catalog.category,
...
async function updateClientFeatureStatus(admin, featureKey, userId, status, notifyUser) {
  ...
  if (featureKey === "notifications") {
    ...
    const { data, error } = await adminSupabase.from(control.table).update({
      status,
      updated_at: nowIso()
    }).eq(control.userColumn || "user_id", userId).select("user_id").maybeSingle();
    if (error) {
      throw error;
    }
    if (!data) {
...
var websiteBlogsFile = path.join(websiteContentRoot, "data", "blogs.json");
var websiteHelpFile = path.join(websiteContentRoot, "data", "help.json");
var websiteLeadFormsFile = websiteContentRoot ? path.join(websiteContentRoot, "data", "lead-form-submissions.json") : "";
var websiteUploadsDir = path.join(websiteContentRoot, "uploads");
var defaultHelpCategories = ["Connektly Overview", "Get Started", "Connect Your Number", "Privacy & Security"];
...
async function callWebsiteAdminApi(endpoint, init = {}) {
...
async function readWebsiteJsonArray(filePath) {
  try {
...
async function writeWebsiteJsonArray(filePath, rows) {
...
function normalizeWebsiteHelpArticle(row) {
  return {
...
function normalizeWebsiteLeadType(value) {
  return normalizeString(value) === "booked_demo" ? "booked_demo" : "lead_inquiry";
}
function normalizeWebsiteLeadFieldValue(value) {
  if (Array.isArray(value)) {
    return value.map((item) => String(item ?? "").trim()).filter(Boolean);
  }
  return String(value ?? "").trim();
}
function normalizeWebsiteLeadFields(value) {
  if (!isRecord(value)) {
    return {};
  }
...
function normalizeWebsiteLeadSubmission(row) {
  return {
...
async function loadWebsiteContentFromLocal() {
  const [blogRows, helpRows] = await Promise.all([
    readWebsiteJsonArray(websiteBlogsFile),
    readWebsiteJsonArray(websiteHelpFile)
  ]);
  const blogs = blogRows.map(normalizeWebsiteBlog).sort((left, right) => Date.parse(right.date) - Date.parse(left.date));
  const helpArticles = helpRows.map(normalizeWebsiteHelpArticle).sort((left, right) => Date.parse(right.date) - Date.parse(left.date));
  const categories = [.../* @__PURE__ */ new Set([...defaultHelpCategories, ...helpArticles.map((article) => article.category).filter(Boolean)])];
  return {
    generatedAt: nowIso(),
...
async function loadWebsiteContent() {
  try {
    return await callWebsiteAdminApi("/content", { cache: "no-store" });
  } catch (error) {
...
async function loadWebsiteLeadFormsFromLocal() {
  const rows = await readWebsiteJsonArray(websiteLeadFormsFile);
  const submissions = rows.map(normalizeWebsiteLeadSubmission).sort((left, right) => Date.parse(right.submittedAt) - Date.parse(left.submittedAt));
...
async function loadWebsiteLeadForms() {
  try {
    return await callWebsiteAdminApi("/lead-form-submissions", { cache: "no-store" });
  } catch (error) {
    if (!canUseLocalWebsiteFiles()) {
      throw error;
    }
...
function requireWebsiteTitle(value, label) {
...
function sortWebsiteRows(rows) {
  return [...rows].sort(
    (left, right) => Date.parse(String(right.updatedAt || right.date || 0)) - Date.parse(String(left.updatedAt || left.date || 0))
  );
}
async function saveWebsiteBlog(admin, body, id) {
  try {
    const response = await callWebsiteAdminApi(
  ...
  const nextBlog = normalizeBlogPayload(body, index >= 0 ? blogs[index] : void 0);
  const nextBlogs = index >= 0 ? blogs.map((blog, blogIndex) => blogIndex === index ? nextBlog : blog) : [nextBlog, ...blogs];
  await writeWebsiteJsonArray(websiteBlogsFile, sortWebsiteRows(nextBlogs));
...
async function deleteWebsiteBlog(admin, id) {
  try {
    const response = await callWebsiteAdminApi(`/content/blogs/${encodeURIComponent(id)}`, {
      method: "DELETE"
    });
  ...
  await writeWebsiteJsonArray(websiteBlogsFile, blogs.filter((blog) => blog.id !== id));
  await recordAdminAudit(admin, "DELETE_WEBSITE_BLOG", null, { id, title: existing.title });
  return loadWebsiteContent();
}
async function saveWebsiteHelpArticle(admin, body, id) {
  try {
    const response = await callWebsiteAdminApi(
  ...
  const nextArticle = normalizeHelpPayload(body, index >= 0 ? articles[index] : void 0);
  const nextArticles = index >= 0 ? articles.map((article, articleIndex) => articleIndex === index ? nextArticle : article) : [nextArticle, ...articles];
  await writeWebsiteJsonArray(websiteHelpFile, sortWebsiteRows(nextArticles));
...
async function deleteWebsiteHelpArticle(admin, id) {
  try {
    const response = await callWebsiteAdminApi(`/content/help/${encodeURIComponent(id)}`, {
      method: "DELETE"
    });
  ...
  await writeWebsiteJsonArray(websiteHelpFile, articles.filter((article) => article.id !== id));
...
async function saveWebsiteMedia(admin, body) {
  try {
    const response = await callWebsiteAdminApi("/content/media", {
      method: "POST",
      body: JSON.stringify(body)
    });
  ...
  const fileName = `${Date.now()}-${crypto.randomInt(1e8, 999999999)}-${sanitizeUploadBaseName(body.fileName)}.${extension}`;
...
app.patch("/api/admin/settings/profile", requireAdmin, async (req, res) => {
  try {
    const admin = req.admin;
    if (!admin) {
      throw new Error("Admin context was not initialized.");
    ...
    if (updates.full_name !== void 0) metadataUpdates.full_name = updates.full_name;
    if (updates.phone !== void 0) metadataUpdates.phone = updates.phone;
    if (updates.organization_name !== void 0) metadataUpdates.organization_name = updates.organization_name;
    if (updates.organization_website !== void 0) metadataUpdates.organization_website = updates.organization_website;
    if (updates.role_title !== void 0) metadataUpdates.role_title = updates.role_title;
...
app.post("/api/admin/settings/profile-photo", requireAdmin, async (req, res) => {
  try {
    const admin = req.admin;
    if (!admin) {
      throw new Error("Admin context was not initialized.");
...
app.patch("/api/admin/settings/account", requireAdmin, async (req, res) => {
  try {
    const admin = req.admin;
    if (!admin) {
      throw new Error("Admin context was not initialized.");
    ...
    const { data, error } = await adminSupabase.auth.admin.updateUserById(admin.id, authUpdates);
    if (error) {
      throw error;
    }
    if (loginEmail) {
      await upsertOwnerProfile({ ...admin, email: loginEmail }, { email: loginEmail });
      await adminSupabase.from("owner_admin_users").update({ email: loginEmail, updated_at: nowIso() }).eq("auth_user_id", admin.id).then((result) => {
...
app.post("/api/admin/admin-users/invite", requireAdmin, requirePrimaryOwner, async (req, res) => {
  try {
    const admin = req.admin;
    if (!admin) {
      throw new Error("Admin context was not initialized.");
    ...
    if (!authUser) {
      ...
      const { data, error: error2 } = await adminSupabase.auth.admin.inviteUserByEmail(email, options);
      if (error2) {
        const message = error2.message.toLowerCase();
        if (!message.includes("already") && !message.includes("registered")) {
          throw error2;
        }
        inviteError = error2.message;
...
app.patch("/api/admin/admin-users/:id", requireAdmin, requirePrimaryOwner, async (req, res) => {
  try {
    const admin = req.admin;
    if (!admin) {
      throw new Error("Admin context was not initialized.");
...
app.delete("/api/admin/admin-users/:id", requireAdmin, requirePrimaryOwner, async (req, res) => {
  try {
    const admin = req.admin;
    if (!admin) {
      throw new Error("Admin context was not initialized.");
...
app.put("/api/admin/plans", requireAdmin, requireAdminPermission("plan_management"), async (req, res) => {
  try {
...
app.patch("/api/admin/platform-settings/:section", requireAdmin, requireAdminPermission("platform_settings"), async (req, res) => {
  try {
...
app.patch("/api/admin/client-features/:featureKey/:userId/status", requireAdmin, requireAdminPermission("global_integrations"), async (req, res) => {
  try {
...
app.post("/api/admin/website-content/media", requireAdmin, requireAdminPermission("website_management"), async (req, res) => {
  try {
...
app.post("/api/admin/website-content/blogs", requireAdmin, requireAdminPermission("website_management"), async (req, res) => {
  try {
...
app.patch("/api/admin/website-content/blogs/:id", requireAdmin, requireAdminPermission("website_management"), async (req, res) => {
  try {
    const id = normalizeString(req.params.id);
    if (!id) {
...
app.delete("/api/admin/website-content/blogs/:id", requireAdmin, requireAdminPermission("website_management"), async (req, res) => {
  try {
    const id = normalizeString(req.params.id);
    if (!id) {
      throw new Error("Blog post id is required.");
...
app.post("/api/admin/website-content/help", requireAdmin, requireAdminPermission("website_management"), async (req, res) => {
  try {
...
app.patch("/api/admin/website-content/help/:id", requireAdmin, requireAdminPermission("website_management"), async (req, res) => {
  try {
    const id = normalizeString(req.params.id);
    if (!id) {
...
app.delete("/api/admin/website-content/help/:id", requireAdmin, requireAdminPermission("website_management"), async (req, res) => {
  try {
    const id = normalizeString(req.params.id);
    if (!id) {
      throw new Error("Help article id is required.");
...
app.post("/api/admin/organizations/:orgId/action", requireAdmin, requireAdminPermission("organizations"), async (req, res) => {
  try {
    const { orgId } = req.params;
...
app.get("/api/admin/users", requireAdmin, requireAdminPermission("global_users"), async (req, res) => {
  try {
    ...
    if (search) {
      users = users.filter(
        (user) => [user.email, user.fullName, user.companyName, user.phone, user.userId].filter(Boolean).some((value) => String(value).toLowerCase().includes(search))
      );
    }
    if (status !== "all") {
...
app.get("/api/admin/users/:userId", requireAdmin, requireAdminPermission("global_users"), async (req, res) => {
  try {
    ...
    const [authUser, profile, metaChannel, instagramChannel, messengerChannel, conversations, messages, calls, callSessions, credits, emailCampaigns, notifications] = await Promise.all([
      getAuthUserById(userId),
      safeSelect("app_profiles", "*", (query) => query.eq("user_id", userId).maybeSingle()),
      safeSelect("meta_channels", "*", (query) => query.eq("user_id", userId).maybeSingle()),
      safeSelect("instagram_channels", "*", (query) => query.eq("user_id", userId).maybeSingle()),
      safeSelect("messenger_channels", "*", (query) => query.eq("user_id", userId).maybeSingle()),
      safeSelect("conversation_threads", "*", (query) => query.eq("user_id", userId).order("updated_at", { ascending: false }).limit(50)),
      safeSelect("conversation_messages", "*", (query) => query.eq("user_id", userId).order("created_at", { ascending: false }).limit(100)),
      safeSelect("call_logs", "*", (query) => query.eq("user_id", userId).order("created_at", { ascending: false }).limit(50)),
      safeSelect("call_sessions", "*", (query) => query.eq("user_id", userId).order("updated_at", { ascending: false }).limit(20)),
      safeSelect("credit_ledger", "*", (query) => query.eq("user_id", userId).order("created_at", { ascending: false }).limit(100)),
      safeSelect("email_campaigns", "*", (query) => query.eq("user_id", userId).order("created_at", { ascending: false }).limit(50)),
      safeSelect("user_notifications", "*", (query) => query.eq("user_id", userId).order("created_at", { ascending: false }).limit(50))
    ]);
    ...
    res.json({
      user: {
        ...
        profile: profileRow,
        displayName: userDisplayName(profileRow || void 0, authUser),
        channels: {
...
app.patch("/api/admin/users/:userId/profile", requireAdmin, requireAdminPermission("global_users"), async (req, res) => {
  try {
    const { userId } = req.params;
    const allowedFields = [
      "full_name",
...
app.post("/api/admin/users/:userId/credits", requireAdmin, requireAdminPermission("global_users"), async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    const { data, error } = await adminSupabase.from("credit_ledger").insert({
      user_id: userId,
      description,
      type,
      amount,
...
app.post("/api/admin/users/:userId/notice", requireAdmin, requireAdminPermission("global_users"), async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    const { data, error } = await adminSupabase.from("user_notifications").insert({
      user_id: userId,
      type: "lead_created",
      title,
      body,
...
app.post("/api/admin/users/:userId/auth", requireAdmin, requireAdminPermission("global_users"), async (req, res) => {
  try {
    const { userId } = req.params;
    const action = normalizeString(req.body.action);
...
app.get("/api/admin/payments", requireAdmin, requireAdminPermission("payments"), async (_req, res) => {
  try {
    const [profiles, creditLedger, paymentEvents] = await Promise.all([
      safeSelect("app_profiles", "*", (query) => query.order("updated_at", { ascending: false }).limit(1e3)),
      safeSelect("credit_ledger", "*", (query) => query.order("created_at", { ascending: false }).limit(500)),
      safeSelect(
        "whatsapp_payment_configuration_events",
        "*",
        (query) => query.order("created_at", { ascending: false }).limit(500)
    ...
    const billingBreakdown = profiles.rows.reduce((acc, row) => {
      const key = normalizeString(row.billing_status) || "unknown";
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
    const planBreakdown = profiles.rows.reduce((acc, row) => {
      const key = normalizeString(row.selected_plan) || "none";
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
    const ledgerTotals = creditLedger.rows.reduce(
      (acc, row) => {
        const amount = normalizeNumber(row.amount);
...
app.get("/api/admin/webhooks", requireAdmin, requireAdminPermission("webhooks"), async (_req, res) => {
  try {
    const [leadConfigs, leadEvents, paymentEvents, messengerChannels, messages, calls, developerWebhookEndpoints] = await Promise.all([
      safeSelect("meta_lead_capture_configs", "*", (query) => query.order("updated_at", { ascending: false }).limit(200)),
      safeSelect("meta_lead_capture_events", "*", (query) => query.order("created_at", { ascending: false }).limit(300)),
      safeSelect(
        "whatsapp_payment_configuration_events",
        "*",
        (query) => query.order("created_at", { ascending: false }).limit(300)
      ),
      safeSelect("messenger_channels", "*", (query) => query.order("updated_at", { ascending: false }).limit(200)),
      safeSelect("conversation_messages", "*", (query) => query.order("created_at", { ascending: false }).limit(200)),
      safeSelect("call_sessions", "*", (query) => query.order("updated_at", { ascending: false }).limit(200)),
      safeSelect("developer_webhook_endpoints", "*", (query) => query.order("updated_at", { ascending: false }).limit(500))
    ]);
    const events = [
    ...
    ].sort((left, right) => Date.parse(right.occurredAt) - Date.parse(left.occurredAt)).slice(0, 400);
...
app.get("/api/admin/webhooks/:webhookId/token", requireAdmin, requireAdminPermission("webhooks"), async (req, res) => {
  try {
