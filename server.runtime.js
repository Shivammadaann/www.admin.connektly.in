...
const allowedEmails = new Set((process.env.ADMIN_ALLOWED_EMAILS || '')
    .split(',')
    .map((value) => value.trim().toLowerCase())
    .filter(Boolean));
const allowedUserIds = new Set((process.env.ADMIN_ALLOWED_USER_IDS || '')
    .split(',')
...
function userDisplayName(profile, authUser) {
    return (normalizeString(profile?.full_name) ||
...
function detectBrowser(userAgent) {
    ...
    if (/Safari\//i.test(value) && !/Chrome\//i.test(value))
        return 'Safari';
    if (/PostmanRuntime/i.test(value))
        return 'Postman';
    if (/curl/i.test(value))
        return 'curl';
...
function detectDevice(userAgent, fallback) {
    ...
    if (/iPad|Tablet/i.test(value))
        return 'Tablet';
    if (/Mobile|Android|iPhone|iPod/i.test(value))
        return 'Mobile';
    if (value)
        return 'Desktop';
    return null;
...
function extractAuditUserAgent(rawPayload, row) {
    ...
    return pickString(row.user_agent, rawPayload.user_agent, rawPayload.userAgent, rawPayload.user_agent_string, rawPayload.userAgentString, traits.user_agent, traits.userAgent, metadata.user_agent, metadata.userAgent, request.user_agent, request.userAgent, requestHeaders['user-agent'], requestHeaders.user_agent, requestHeaders.userAgent, headers['user-agent'], headers.user_agent, headers.userAgent, context.user_agent, context.userAgent, contextUserAgent.original);
}
function extractAuditIpAddress(rawPayload, row) {
    ...
    const requestHeaders = nestedRecord(request, 'headers');
    const headers = nestedRecord(rawPayload, 'headers');
    return pickString(row.ip_address, rawPayload.ip_address, rawPayload.ipAddress, rawPayload.ip, traits.ip_address, traits.ipAddress, traits.ip, metadata.ip_address, metadata.ipAddress, metadata.ip, request.ip_address, request.ipAddress, request.ip, requestHeaders['x-forwarded-for'], requestHeaders['x-real-ip'], requestHeaders['cf-connecting-ip'], requestHeaders['x-client-ip'], headers['x-forwarded-for'], headers['x-real-ip'], headers['cf-connecting-ip'], headers['x-client-ip']);
}
function extractAuditDevice(rawPayload, row, userAgent) {
    const traits = nestedRecord(rawPayload, 'traits');
    const metadata = nestedRecord(rawPayload, 'metadata');
    const request = nestedRecord(rawPayload, 'request');
    return detectDevice(userAgent, pickString(row.device, rawPayload.device, rawPayload.device_type, rawPayload.deviceType, traits.device, traits.device_type, traits.deviceType, metadata.device, metadata.device_type, metadata.deviceType, request.device));
}
...
function normalizeOwnerNotifications(value) {
    ...
    return Object.fromEntries(Object.entries(defaultOwnerNotifications).map(([key, fallback]) => [key, typeof incoming[key] === 'boolean' ? incoming[key] : fallback]));
}
...
async function getAdminAccessForUser(user) {
    ...
    if (byUserId.error && isMissingRelationError(byUserId.error)) {
        const legacyAllowed = allowedUserIds.has(user.id) ||
...
const platformSettingSections = Object.keys(defaultPlatformSettings);
        ...
        return {
            plans: plans.map((plan) => ({
                ...
                features: Array.isArray(plan.features)
                    ? plan.features.map((feature) => normalizeString(feature)).filter(Boolean)
                    : [],
    ...
    if (section === 'email_templates') {
        ...
        return {
            ...
            customTriggers: customTriggers.map((trigger) => {
                const triggerKey = normalizeString(trigger.triggerKey) ||
...
function buildOwnerSettings(admin, profile, warning, authUser = admin.user) {
    ...
    const fullName = normalizeString(profile?.full_name) ||
        ...
        'Owner';
    const organizationName = normalizeString(profile?.organization_name) ||
...
function normalizePlanRank(plan) {
...
function mapRazorpayPaymentMethod(payment, invoice, subscription = null, error) {
    ...
    const cardLast4 = normalizeString(card.last4) ||
    ...
    const mandateEnabled = Boolean(tokenId ||
...
async function resolveSubscriptionPaymentMethod(subscriptionId) {
    ...
    try {
        ...
        if (!invoice || !paymentId) {
            return mapRazorpayPaymentMethod(null, invoice, isRecord(subscription) ? subscription : null, 'No paid invoice payment was found for this subscription.');
        }
...
function isChurnedProfile(row) {
    ...
    return ['cancelled', 'canceled', 'deleted', 'churned', 'suspended'].some((status) => billingStatus.includes(status) || accountStatus.includes(status));
}
...
function logStatus(row) {
    return (normalizeString(row.status) ||
...
function logErrorType(row) {
    ...
    if (!value)
        return null;
    if (value.includes('auth') || value.includes('token') || value.includes('permission'))
        return 'auth';
    if (value.includes('rate') || value.includes('limit') || value.includes('quota'))
        return 'rate_limit';
    if (value.includes('timeout'))
        return 'timeout';
    if (value.includes('disconnect'))
        return 'disconnection';
    if (value.includes('payment') || value.includes('declin') || value.includes('past_due'))
        return 'payment';
...
function logDetail(row) {
    return (normalizeString(row.error_message) ||
...
function mapWhatsAppTwoStepVerification(row) {
    ...
    const liveIsEnabled = typeof twoStep.isPinEnabled === 'boolean'
    ...
    const inferredEnabled = Boolean(enabledAt) &&
...
function buildOrganizationRows(core, authUsers) {
    ...
    return users.map((user) => {
        ...
        const unpaidHighUsage = ['free', 'trialing', 'none', ''].includes(String(plan).toLowerCase()) &&
...
function buildOrganizationDetail(core, authUsers, orgId) {
    ...
    return {
        ...
        usageStats: {
            ...
            apiUsage: core.messages.filter(ownerMatches).length +
...
function metaErrorMessage(payload) {
    ...
    if (isRecord(payloadRecord.error)) {
        ...
        return (normalizeString(error.error_user_msg) ||
...
async function getOrgWhatsAppChannel(orgId) {
    ...
    if (error)
        throw error;
    if (!data)
        throw new Error('This organization does not have a WhatsApp Business number linked.');
    const row = data;
    return {
        row,
...
async function runWhatsAppWebhookAction(orgId, action, req) {
    ...
    if (!wabaId)
        throw new Error('This WhatsApp channel is missing its WABA ID.');
...
async function requestWhatsAppVerificationCode(orgId, codeMethod, language) {
    ...
    if (!phoneNumberId)
        throw new Error('This WhatsApp channel is missing its phone number ID.');
    await metaRequest({
        accessToken,
...
function mapLivePayload(table, payload) {
    ...
    return {
        ...
        description: normalizeString(row.last_message_text) ||
        ...
        status: normalizeString(row.processing_status) ||
...
function startRealtimeBridge() {
    ...
    for (const table of tables) {
        channel = channel.on('postgres_changes', { event: '*', schema: 'public', table }, (payload) => pushLiveEvent(mapLivePayload(table, payload)));
    }
...
async function loadCoreData() {
    const [profiles, metaChannels, instagramChannels, messengerChannels, metaAdsIntegrations, metaLeadCaptureConfigs, metaFlows, flowSubmissions, conversationalAutomationConfigs, automationRules, threads, messages, callLogs, callSessions, leadEvents, paymentEvents, emailCampaigns, creditLedger, notifications, notificationPreferences, emailConnections, emailTemplates, woocommerceConnections, developerApiCredentials, developerWebhookEndpoints, workspaceTeamMembers, templates,] = await Promise.all([
        safeSelect('app_profiles', '*', (query) => query.order('created_at', { ascending: false }).limit(1000)),
        ...
        safeSelect('meta_flows', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        safeSelect('flow_submissions', '*', (query) => query.order('submitted_at', { ascending: false }).limit(1000)),
        safeSelect('meta_conversational_automation_configs', '*'),
        safeSelect('automation_rules', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        safeSelect('conversation_threads', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        safeSelect('conversation_messages', '*', (query) => query.order('created_at', { ascending: false }).limit(1000)),
        safeSelect('call_logs', '*', (query) => query.order('created_at', { ascending: false }).limit(1000)),
        safeSelect('call_sessions', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        safeSelect('meta_lead_capture_events', '*', (query) => query.order('created_at', { ascending: false }).limit(500)),
        safeSelect('whatsapp_payment_configuration_events', '*', (query) => query.order('created_at', { ascending: false }).limit(500)),
        safeSelect('email_campaigns', '*', (query) => query.order('created_at', { ascending: false }).limit(1000)),
        safeSelect('credit_ledger', '*', (query) => query.order('created_at', { ascending: false }).limit(2000)),
        ...
        safeSelect('email_templates', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        safeSelect('woocommerce_connections', '*'),
        safeSelect('developer_api_credentials', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        safeSelect('developer_webhook_endpoints', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        safeSelect('workspace_team_members', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        safeSelect('meta_templates', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
    ]);
    return {
...
function webhookEndpointUrl(row) {
    return (normalizeString(row.url) ||
...
function webhookEndpointSecret(row) {
    return (normalizeString(row.signing_secret) ||
...
function developerWebhookReference(row, index) {
    ...
    return {
        ...
        purpose: normalizeString(row.description) ||
...
async function loadWebhookTokenValues(webhookId) {
    ...
    if (webhookId === 'meta-lead-capture') {
        const configs = await safeSelect('meta_lead_capture_configs', '*', (query) => query.order('updated_at', { ascending: false }).limit(500));
        return configs.rows
            .filter((row) => isActiveWebhookRow(row))
            .map((row, index) => webhookTokenValue({
            id: normalizeString(row.id) || `meta-lead-token:${index}`,
            label: normalizeString(row.page_name) ||
...
function userActivityTimestamp(row, fallbackKey = 'created_at') {
    return (normalizeString(row.onboarding_completed_at) ||
    ...
    const payments = core.paymentEvents
        ...
        return {
            ...
            metadata: {
                ...
                reference: normalizeString(row.razorpay_payment_id) ||
...
function buildOverview(core, authUsers, health) {
    ...
    return {
        ...
        metrics: {
            ...
            activeOrganizations: organizations.filter((organization) => ['active', 'paid', 'trialing', 'trial'].some((status) => String(organization.status || '').toLowerCase().includes(status))).length,
        ...
        charts: {
            revenueGrowth: months.map((date) => ({
                label: monthLabel(date),
                value: Math.round(core.profiles
                    .filter((row) => {
                    const createdAt = timestampFromRow(row, 'created_at');
                    const updatedAt = timestampFromRow(row, 'updated_at');
                    ...
                    .reduce((total, row) => total + planMonthlyValue(row), 0) * 100) / 100,
            })),
            customerMovement,
...
function buildLogsMonitoring(core, health) {
    const apiLogs = sortLogs([
        ...
        ...core.metaChannels.map((row) => buildLogEntry({
            row,
        ...
        ...core.instagramChannels.map((row) => buildLogEntry({
            row,
        ...
        ...core.messengerChannels.map((row) => buildLogEntry({
            row,
        ...
        ...core.metaAdsIntegrations.map((row) => buildLogEntry({
            row,
        ...
        ...core.metaLeadCaptureConfigs.map((row) => buildLogEntry({
            row,
        ...
        ...core.metaFlows.map((row) => buildLogEntry({
            row,
        ...
        ...core.flowSubmissions.map((row) => buildLogEntry({
            row,
        ...
        ...core.templates.map((row) => buildLogEntry({
            row,
        ...
        ...core.leadEvents.map((row) => buildLogEntry({
            row,
            idPrefix: 'lead-api',
            category: 'api',
            source: 'Meta Graph API',
        ...
        ...core.paymentEvents.map((row) => buildLogEntry({
            row,
        ...
        ...core.messages.map((row) => buildLogEntry({
            row,
        ...
        ...core.callLogs.map((row) => buildLogEntry({
            row,
        ...
        ...core.callSessions.map((row) => buildLogEntry({
            row,
            idPrefix: 'call-api',
            category: 'api',
            source: 'Meta Graph API',
    ...
    const messageDeliveryLogs = sortLogs(core.messages.map((row) => buildLogEntry({
        row,
    ...
    return {
        ...
        integrations: [
            ...
            },
            {
                ...
                status: emailUnhealthy === 0 && failedEmailCampaigns === 0 && (core.emailConnections.length > 0 || core.emailCampaigns.length > 0)
                    ? 'healthy'
...
const clientFeatureKeys = Object.keys(clientFeatureCatalog);
const clientFeatureStatusControls = {
...
function getWorkspaceIdentity(userId, profileById, authById) {
    ...
    const authUser = authById.get(userId);
    return {
        organizationName: normalizeString(profile?.company_name) ||
...
function collectFeatureRisks(rows, status, extras = []) {
    ...
    for (const row of rows) {
        const lastError = normalizeString(row.last_error) ||
            normalizeString(row.error_message) ||
...
function latestByUpdatedAt(rows) {
    return rows
        .slice()
        .sort((left, right) => Date.parse(String(right.updated_at || right.created_at || right.submitted_at || 0)) -
        Date.parse(String(left.updated_at || left.created_at || left.submitted_at || 0)))[0] || null;
}
function buildClientFeatureOperations(core, authUsers) {
    ...
    for (const row of core.metaChannels) {
        ...
        addRecord({
            ...
            userId,
            status: normalizeString(row.status) || 'connected',
            detail: normalizeString(row.display_phone_number) ||
    ...
    for (const row of core.metaAdsIntegrations) {
        ...
        addRecord({
            ...
            detail: normalizeString(row.ad_account_name) ||
    ...
    for (const [userId, rows] of paymentEventsByUser) {
        ...
        addRecord({
            ...
            detail: normalizeString(latest?.configuration_name) ||
    ...
    for (const userId of collectGroupOwnerIds(emailConnectionsByUser, emailTemplatesByUser, emailCampaignsByUser)) {
        ...
        addRecord({
            ...
            detail: normalizeString(latestConnection?.email_address) ||
...
function sortWebsiteRows(rows) {
    return [...rows].sort((left, right) => Date.parse(String(right.updatedAt || right.date || 0)) - Date.parse(String(left.updatedAt || left.date || 0)));
}
async function saveWebsiteBlog(admin, body, id) {
    try {
        const response = await callWebsiteAdminApi(id ? `/content/blogs/${encodeURIComponent(id)}` : '/content/blogs', {
...
async function saveWebsiteHelpArticle(admin, body, id) {
    try {
        const response = await callWebsiteAdminApi(id ? `/content/help/${encodeURIComponent(id)}` : '/content/help', {
            method: id ? 'PATCH' : 'POST',
            body: JSON.stringify(body),
        });
...
app.patch('/api/admin/settings/profile', requireAdmin, async (req, res) => {
    try {
        ...
        if (updates.organization_name !== undefined)
            metadataUpdates.organization_name = updates.organization_name;
        if (updates.organization_website !== undefined)
            metadataUpdates.organization_website = updates.organization_website;
        if (updates.role_title !== undefined)
            metadataUpdates.role_title = updates.role_title;
...
app.post('/api/admin/admin-users/invite', requireAdmin, requirePrimaryOwner, async (req, res) => {
    try {
        ...
        if (!authUser) {
            ...
            const { data, error } = await adminSupabase.auth.admin.inviteUserByEmail(email, options);
            if (error) {
                const message = error.message.toLowerCase();
...
app.get('/api/admin/logs', requireAdmin, requireAnyAdminPermission(['logs_monitoring', 'webhooks', 'server_status', 'security_audit']), async (_req, res) => {
    try {
...
app.get('/api/admin/organizations', requireAdmin, requireAdminPermission('organizations'), async (req, res) => {
    try {
        ...
        res.json({
            organizations: organizations.sort((left, right) => Date.parse(String(right.updatedAt || right.createdAt || 0)) - Date.parse(String(left.updatedAt || left.createdAt || 0))),
            summary: {
...
app.get('/api/admin/users', requireAdmin, requireAdminPermission('global_users'), async (req, res) => {
    try {
        ...
        if (search) {
            users = users.filter((user) => [user.email, user.fullName, user.companyName, user.phone, user.userId]
                .filter(Boolean)
                .some((value) => String(value).toLowerCase().includes(search)));
        }
        if (status !== 'all') {
...
app.get('/api/admin/payments', requireAdmin, requireAdminPermission('payments'), async (_req, res) => {
    try {
        const [profiles, creditLedger, paymentEvents] = await Promise.all([
            safeSelect('app_profiles', '*', (query) => query.order('updated_at', { ascending: false }).limit(1000)),
        ...
        const ledgerTotals = creditLedger.rows.reduce((acc, row) => {
            const amount = normalizeNumber(row.amount);
...
app.get('/api/admin/webhooks', requireAdmin, requireAdminPermission('webhooks'), async (_req, res) => {
    try {
        const [leadConfigs, leadEvents, paymentEvents, messengerChannels, messages, calls, developerWebhookEndpoints] = await Promise.all([
            ...
            safeSelect('whatsapp_payment_configuration_events', '*', (query) => query.order('created_at', { ascending: false }).limit(300)),
...
app.get('/api/admin/audit', requireAdmin, requireAdminPermission('security_audit'), async (_req, res) => {
    try {
        const audit = await safeSelect('owner_admin_audit_events', '*', (query) => query.order('created_at', { ascending: false }).limit(200));
        res.json({
