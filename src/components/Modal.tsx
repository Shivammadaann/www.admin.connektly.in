...
export default function Modal({ title, description, isOpen, onClose, children, maxWidthClass = 'max-w-6xl' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 p-3 backdrop-blur-sm sm:p-6">
      ...
      <section
        className={`relative mx-auto my-4 overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-[0_28px_90px_rgba(15,23,42,0.24)] ${maxWidthClass}`}
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-gray-100 bg-white/95 px-5 py-4 backdrop-blur sm:px-6">
          <div className="min-w-0">
            <h2 className="truncate text-lg font-bold tracking-tight text-gray-950">{title}</h2>
